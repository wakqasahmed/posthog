...
async def _kill_query_on_cancellation(client: ClickHouseClient, query_id: str) -> typing.AsyncIterator[None]:
    ...
    The kill is best-effort: shielded so the cancellation in flight doesn't abort it, timeout-bounded
    so a slow ClickHouse can't hold it up, and failures are swallowed since the query's execution
    time limit is the backstop.
    """
    logger = LOGGER.bind(query_id=query_id)
    try:
        yield
        ...
        try:
            await asyncio.wait_for(asyncio.shield(client.acancel_query(query_id)), timeout=30)
        except asyncio.CancelledError:
