...
def enqueue_or_start_workflow(task_run: TaskRun, *, options: WorkflowDispatchOptions) -> None:
    ...
    if shadow_enabled or async_enabled:
    ...
    def start_synchronously() -> None:
        ...
        execute_task_processing_workflow(
            ...
            posthog_mcp_scopes=options.posthog_mcp_scopes,
            prewarmed=options.prewarmed,
            workflow_id_prefix=options.workflow_id_prefix,
            initial_message=options.initial_message,
        )

    transaction.on_commit(start_synchronously)
